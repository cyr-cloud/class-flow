...
export default function SurveySlide({ sessionId, content, teacher }: { sessionId: string; content: SurveyContent; teacher: boolean }) {
  ...
  return <section aria-label={LABELS[content.display]} className="flex h-full min-h-80 min-w-0 flex-col rounded-xl bg-[#faf7f2] p-4 text-ink sm:p-8">
    ...
    <div className="min-h-32 flex-1 overflow-y-auto" aria-label="실시간 응답 결과">
      {cards ? <div className="columns-1 gap-4 md:columns-2 xl:columns-3">{responses.length ? responses.map(r => <article key={r.responderId} className="mb-4 break-inside-avoid whitespace-pre-wrap rounded-2xl bg-[#eae9e7] p-5 text-base leading-relaxed [overflow-wrap:anywhere] sm:text-xl">{r.text}</article>) : <p className="p-8 text-center text-mute">참가자들의 답변이 카드로 나타납니다.</p>}</div> : <div className={content.display === "bar" ? "space-y-4" : "flex flex-col items-center gap-6 lg:flex-row"}>
