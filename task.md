...
## 수업이 끝난 뒤에도 남기기 — «내 강의실»

수업이 끝나면 지금은 사실상 사라진다. 강사가 노션 페이지를 평생 열어두는 이유가 있다 —
학생이 나중에 다시 보고, 못 따라간 실습을 그때 해보기 때문이다. 링크가 살아 있는 것이
자료를 나눠주는 것보다 훨씬 쓸모 있다.

**단순히 «목록 화면 하나»가 아니다.** 아래가 같이 풀려야 한다.

- [ ] **학생을 다시 알아보기**
      지금 응답자 식별은 `sessionStorage`라 탭을 닫으면 끊긴다. 다음에 와서 «내 강의실»을
      보려면 브라우저에 남는 식별자가 필요하다. 닉네임 항목(Phase 3)과 같이 풀면 된다.
      기기를 바꾸면 못 찾는 건 한계로 남는다 — 로그인을 넣을지는 그때 판단.
- [ ] **수업 끝내기 + 지난 수업 모드**
      다시 보기는 실시간이 아니다. 강사를 따라갈 게 없으니 **학생이 스스로 넘겨야** 한다.
      지금 학생 화면은 `currentSlide`를 그대로 따라가게 되어 있어서, 끝난 수업에서는
      스스로 넘기는 모드로 갈라져야 한다. 강사에게는 «수업 종료» 같은 자리가 필요하고.
- [ ] **강의실 목록 화면 — 지금 하는 수업과 지난 수업을 한눈에 가르기**
      학생: 내가 들어갔던 수업들 (날짜 · 강의명 · 내가 올린 결과물)
      강사: 내가 연 수업들 (참여 인원 · 결과물 수 · 다시 열기)
      진행 중인 수업은 오른쪽 위에 **«LIVE»** 를 달고(점이 깜빡이는 형태),
      끝난 수업은 **«복습»** 태그를 단다. 목록에서도, 수업 화면 안에서도 같은 표시를 쓴다 —
      학생이 지금 들어간 곳이 «따라가는 자리»인지 «혼자 보는 자리»인지 한눈에 알아야
      강사를 기다리다 마는 일이 없다.
      LIVE는 «강사가 열어둔 상태»로 판단한다. 강사가 창을 닫고 잠수했는데 계속 LIVE면
      거짓말이 되므로, 마지막 활동 시각(`session.updatedAt`)이 한참 지났으면 자동으로
      «복습»으로 내려가야 한다.
- [ ] **오래 두는 비용** ← 이게 진짜 제약
      수업 하나에 슬라이드 PDF(6MB)와 결과물 이미지가 딸려 있다. 무료 한도가
      Blob 1GB · Redis 256MB라, **수업 100회쯤이면 찬다.**
      정할 것: 얼마나 오래 둘지(1년?), 오래된 수업의 이미지를 줄일지,
      아니면 유료로 올릴지. «평생»을 약속하려면 비용이 얼마인지 먼저 계산해야 한다.
      Redis는 수업 기록용으로는 비싼 자리다 — 지난 수업은 더 싼 곳(Postgres 등)으로
      옮기고 Redis에는 진행 중인 수업만 두는 쪽이 맞다.
