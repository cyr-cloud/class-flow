...
export const quizTypes = ["fact", "blank", "matching", "sequence", "calculation", "reading", "negative", "ox"] as const;
export const AiQuizItem = z.object({
  ...
  answers: z.array(z.number().int()).describe("정답의 0-based index. 반드시 하나 이상"),
});
...
export function validateAiQuizzes(items: GeneratedQuiz[], history: PreviousQuiz[], slideNo: number) {
  ...
  normalized.forEach((item, i) => {
    ...
    if (!validAnswers) fail("정답 번호가 없거나 중복·범위 오류가 있습니다.");
    if (validAnswers && !ox) {
